...
class DataIngestion:
    ...
    def download_file(self):
        if not os.path.exists(self.config.data_dir):
            ...
            logger.info(f"File already exists of size: {get_size(Path(self.config.data_dir))}")
        
        temp_df = pd.read_csv(self.config.data_dir)
        temp_df.to_csv(self.config.raw_data_dir)